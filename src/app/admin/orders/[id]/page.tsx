...
import { ArrowLeft, UserCircle, Hotel as HotelIcon, Calendar, Hash, DollarSign, List, Building } from "lucide-react";
...
import type { Order, Hotel, User } from "@/lib/types";
...
export default function AdminOrderDetailPage() {
    ...
    const fetchOrderDetails = useCallback(async () => {
        ...
        try {
            ...
            const [hotelData, customerData, ownerData] = await Promise.all([
                getHotel(orderData.hotelId),
                getUser(orderData.customerId),
                getUser(orderData.ownerId)
            ]);
            setHotel(hotelData);
                    ...
                    <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
                        <div className="flex items-start gap-3">
                         ...
                         <div className="flex items-start gap-3">
                            ...
                            <div>
                                <p className="text-muted-foreground">Hotel Owner</p>
                                <p className="font-medium">{owner?.name || 'N/A'}</p>
                                <p className="font-mono text-xs">{order.ownerId}</p>
                            </div>
                        </div>
                         <div className="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p className="font-medium">{hotel?.name || 'N/A'}</p>
                                <p className="font-mono text-xs">{order.hotelId}</p>
                            </div>
                        </div>
                        <div className="flex items-start gap-3">
